...
import { ITasksRepository, TaskQueryOptions } from '../repositories/interfaces/tasks.repository.interface';
import { IListsRepository } from '../repositories/interfaces/lists.repository.interface';
import { Task, CreateTaskInput, UpdateTaskInput, TaskSummary } from '../models/task.model';
import { ValidationError, NotFoundError, CustomError } from '../utils/errors';
...
export class TasksService {
   ...
   */
  async getListTasks(listId: string, userId: string, options?: TaskQueryOptions): Promise<Task[]> {
    try {
   ...
   * @throws {CustomError} When retrieval fails
   */
  async getUserTasks(userId: string, options?: TaskQueryOptions): Promise<Task[]> {
    try {
      if (!userId || !userId.trim()) {
        throw new ValidationError('User ID is required');
      }
   ...
   * @throws {CustomError} When update fails
   */
  async markTaskComplete(taskId: string, userId: string): Promise<Task> {
    try {
      // Validate input parameters
      if (!taskId || !taskId.trim()) {
        throw new ValidationError('Task ID is required');
      ...
      throw new CustomError('Failed to mark task as complete', 500);
    }
  }
   ...
   * @throws {CustomError} When update fails
   */
  async markTaskIncomplete(taskId: string, userId: string): Promise<Task> {
    try {
      // Validate input parameters
      if (!taskId || !taskId.trim()) {
        throw new ValidationError('Task ID is required');
      ...
      if (!existingTask.isCompleted) {
        throw new ValidationError('Task is already incomplete');
      }
      ...
      throw new CustomError('Failed to mark task as incomplete', 500);
    }
  }
   ...
   * @throws {CustomError} When retrieval fails
   */
  async getTasksSortedByDeadline(userId: string, options?: DeadlineSortOptions): Promise<Task[]> {
    try {
      if (!userId || !userId.trim()) {
        throw new ValidationError('User ID is required');
      }
      ...
      throw new CustomError('Failed to retrieve tasks sorted by deadline', 500);
    }
  }
  ...
  async getTaskStatistics(userId: string): Promise<TaskStatistics> {
    try {
      ...
      throw new CustomError('Failed to retrieve task statistics', 500);
    }
  }
   ...
   * @private
   */
  private validateUpdateTaskInput(updates: UpdateTaskInput): void {
   ...
   * @private
   */
  private validateDeadline(deadline: string): void {
    ...
    if (deadlineDate < oneDayAgo) {
      throw new ValidationError('Deadline cannot be more than 24 hours in the past');
    }
  }
   ...
   * @private
   */
  private async verifyListOwnership(listId: string, userId: string): Promise<void> {
    const list = await this.listsRepository.findById(listId, userId);
    if (!list) {
      throw new NotFoundError('List not found or you do not have permission to access it');
    }
  }
   ...
   * @private
   */
  private async enforceTaskCreationRules(taskData: CreateTaskInput): Promise<void> {
    ...
    if (listTasks.length >= MAX_TASKS_PER_LIST) {
      throw new CustomError(`Cannot create more than ${MAX_TASKS_PER_LIST} tasks per list`, 400);
    }
  }
   ...
   * @private
   */
  private async enforceTaskUpdateRules(existingTask: Task, updates: UpdateTaskInput): Promise<void> {
    ...
    if (updates.isCompleted === false && 
      ...
      throw new CustomError('Cannot mark overdue high-priority tasks as incomplete', 400);
    }
  }
   ...
   * @private
   */
  private async validateTaskDeletion(task: Task): Promise<void> {
    ...
    if (task.isCompleted && 
      ...
      if (completedDate > oneDayAgo) {
        throw new CustomError('Cannot delete recently completed high-priority tasks', 400);
      }
    }
  }
  ...
  private enrichTaskData(task: Task): Task {
    ...
    if (task.deadline) {
      ...
      (enriched as any).daysUntilDue = diffDays;
      (enriched as any).isOverdue = diffDays < 0 && !task.isCompleted;
      (enriched as any).isDueSoon = diffDays >= 0 && diffDays <= 3 && !task.isCompleted;
    }
   ...
   * @private
   */
  private logTaskDeletion(task: Task, userId: string): void {
