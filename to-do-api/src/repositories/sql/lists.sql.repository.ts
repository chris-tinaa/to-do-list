 ...
 */
export class ListsSqlRepository implements IListsRepository {
  ...
  async update(id: string, updates: UpdateListInput, userId: string): Promise<List> {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        throw new NotFoundError('List not found or user does not own this list');
      }
   ...
   */
  private mapRowToList(row: any): Omit<List, 'taskCount' | 'completedTaskCount'> {
    return {
      id: row.id,
      userId: row.user_id,
      name: row.name,
   ...
   */
  private mapRowToListWithCounts(row: any): List {
    return {
      id: row.id,
      userId: row.user_id,
      name: row.name,
