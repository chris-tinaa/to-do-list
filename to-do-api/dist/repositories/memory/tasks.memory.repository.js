...
exports.TasksMemoryRepository = void 0;
...
class TasksMemoryRepository {
    ...
    async findByListId(listId, userId) {
        try {
            ...
            return listTasks.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        }
        catch (error) {
            if (error instanceof errors_1.ValidationError) {
                throw error;
    ...
    async findById(id, userId) {
        try {
            ...
            if (task.userId !== userId) {
                throw new errors_1.CustomError('User does not own this task', 403);
            }
            return task;
        }
        catch (error) {
    ...
    async findDueThisWeek(userId) {
        try {
            ...
            return dueThisWeek.sort((a, b) => new Date(a.deadline).getTime() - new Date(b.deadline).getTime());
        }
        catch (error) {
            if (error instanceof errors_1.ValidationError) {
                throw error;
...
exports.TasksMemoryRepository = TasksMemoryRepository;
